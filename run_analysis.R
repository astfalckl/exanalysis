...
Xstar <- as.numeric(sst_update$update_tbl$Eadj2)
...
# saveRDS(sic_update, "../inter/sic_update.RDS")
# sic_update <- readRDS("../inter/sic_update.RDS")
...
p_data_sst <- sst_data$data %>%
  ...
  scale_color_scico(expression(degree~"C"), direction = -1, palette = "roma") +
  ...
  coord_fixed() +
  theme_bw() +
  guides(fill = guide_colourbar(barwidth = 100, barheight = 0.1), shape = "none") +
  theme(
...
ggsave(
  "../resubmission2/overleaf/images/data_sst.pdf", 
  p_data_sst, width = 160, height = 100, units = "mm", dpi = 300
)
...
# This recreates the data object but filters for the pseudo data
...
ggsave(
  "../resubmission2/overleaf/images/sst_adj.pdf", 
  p_all_sst, width = 140, height = 165, units = "mm", dpi = 300
)
...
p1 <- lapply(1:length(model_names), function(i){
  tibble(
    x = x_tmp,
...
p2 <- lapply(1:length(model_names), function(i){
  tibble(
    x = x_tmp,
    y = calc_X(x_tmp, spline_params) %*% matrix(betas[,i]) %>% as.numeric(),
    var = diag(calc_X(x_tmp, spline_params) %*% 
...
p3 <- lapply(1:length(model_names), function(i){
  tibble(
    x = x_tmp,
    y = calc_X(x_tmp, spline_params) %*% matrix(betas[,i]) %>% as.numeric(),
    var = diag(calc_X(x_tmp, spline_params) %*% 
...
p4 <- lapply(1:length(model_names), function(i){
  tibble(
    x = x_tmp,
    y = calc_X(x_tmp, spline_params) %*% matrix(betas[,i]) %>% as.numeric(),
    var = diag(calc_X(x_tmp, spline_params) %*% 
...
p_map <- ggplot(coords_select_all) +
  ...
  geom_text(aes(x = lon, y = lat, label = label), hjust=-0.5, vjust=0.2, colour = "red") +
  theme_bw() +
  coord_fixed() +
  theme(axis.title = element_blank())

# plot_locations <- p1 + p2 + p3 + p4
plot_locations2 <- p1 | p2 | p3 | p4

ggsave(
  "../resubmission2/overleaf/images/ind_fits2.pdf",
  plot_locations2, height = 2.5, width = 7.5
)
ggsave(
  "../resubmission2/overleaf/images/map.pdf",
  p_map, width = 5, height = 2.7
)
...
p_ice_adj_var_second <- sic_update$update_tbl %>%
  ...
  ggplot() +
    ...
    scico::scale_fill_scico(
      NULL, direction = 1, palette = "lajolla",
      # limits = c(0, .1), breaks = c(0, 0.05, 0.1)
      limits = c(0, 0.5), breaks = c(0, 0.25, 0.5)
    ) +
    coord_fixed() +
    theme_bw() +
...
ggsave(
  "../resubmission2/overleaf/images/ice_adj.pdf", 
  p_ice_adj, width = 140, height = 165, units = "mm", dpi = 300
)
...
sym_tmp <- 0.5 * (sic_update$V_param_update + Matrix::t(sic_update$V_param_update))
sic_chol <- Matrix::chol(sym_tmp)

sample_ice <- Matrix::t(Matrix::Matrix(rnorm(sst_update$simulation$n*5))) %*% sic_chol
...
ggsave(
  "../resubmission2/overleaf/images/samples.pdf",
  p_samples, width = 140, height = 110, units = "mm", dpi = 300
)
...
callouts <- (canada_plot + greenland_plot) / 
(hudson_plot + pacific_plot)

p_glacier_runs <- location_plot / callouts

callouts <- canada_plot | greenland_plot | hudson_plot | pacific_plot

ggsave(
  "../resubmission2/overleaf/images/glacier_callouts.pdf",
  callouts, height = 2.5, width = 7.5
)
ggsave(
  "../resubmission2/overleaf/images/glacier_locations.pdf",
  location_plot, width = 5, height = 2.1
)